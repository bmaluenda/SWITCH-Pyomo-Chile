...
def _save_generic_results(instance, outdir):
    ...
    for var in instance.component_objects():
        ...
        with open(output_file, 'wb') as fh:
            ini_tuple = datetime.datetime.now()
            ...
            end_tuple = datetime.datetime.now()
            print "Variable %s took %d seconds..." %(var.name, (end_tuple-ini_tuple).microseconds)
    ...
    for var in instance.component_objects():
        ...
        index_name = var.index_set().name
        output_file = os.path.join(outdir, 'Sorted%s.tab' % var.name)
        with open(output_file, 'wb') as fh:
            ini_tuple = datetime.datetime.now()
            writer = csv.writer(fh, dialect='ampl-tab')
            # Write column headings
            ...
            end_tuple = datetime.datetime.now()
            print "Variable Sorted%s took %d seconds..." %(var.name, (end_tuple-ini_tuple).microseconds)
